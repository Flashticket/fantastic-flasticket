...
<div class="el_booking_detail">
			...
			<label>
				...
				<select name="<?php echo $this->get_mb_name('id_event'); ?>" class="id_event">
					...
					<?php foreach ( $events as $key => $value ) { ?>
						<option value="<?php echo $value->ID; ?>"<?php selected( $value->ID, $id_event ); ?>>
				...
				<strong><?php esc_html_e( "Event Calendar *",  "eventlist" ); ?>: </strong>
					<select name="<?php echo OVA_METABOX_EVENT.'id_cal' ?>" >
						<?php foreach ( $list_calendar as $key => $value ) { ?>
							<option value="<?php echo $value['calendar_id']; ?>"<?php selected( $value['calendar_id'], $id_cal ); ?>>
						...
						<?php if ( $field['type'] === 'file' ) { ?>
							<span class="el-ckf-file">
								<a href="<?php echo esc_url( $data_checkout_field[$key] ); ?>" target="_blank">
									<?php echo wp_basename( $data_checkout_field[$key] ); ?>
								</a>
							</span>
						<?php } ?>
					</label>
	...
	<input
		...
		name="<?php echo $this->get_mb_name( 'data_checkout_field' ); ?>"
	...
	<div class="ova_row">
		<div class="wp-button">
			<button class="button create-ticket-send-mail" data-nonce="<?php echo wp_create_nonce( 'el_create_send_ticket_nonce' ); ?>" data-id-booking="<?php echo esc_attr($post->ID); ?>">
				<?php esc_html_e( "Create And Send Ticket", "eventlist" ); ?>
			</button>
			<?php if ( $pdf_invoices === 'yes' ): ?>
				<button
					class="button create-invoice" 
					data-nonce="<?php echo wp_create_nonce( 'el_create_invoice_nonce' ); ?>" 
					data-booking-id="<?php echo esc_attr( $post->ID ); ?>">
					<?php esc_html_e( "View Invoice", "eventlist" ); ?>
				</button>
				<button
					class="button send-invoice" 
					data-nonce="<?php echo wp_create_nonce( 'el_send_invoice_nonce' ); ?>" 
					data-booking-id="<?php echo esc_attr( $post->ID ); ?>">
